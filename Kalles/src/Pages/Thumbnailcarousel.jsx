...
const Thumbnailcarousel = () => {
  ...
  const productImages = [
    'https://kalles-5.myshopify.com/cdn/shop/files/ragbo4535819dd2_q1_2-0_8fc35ffb-cb17-4bd0-8dba-1bf9c36588e8.jpg?v=1717404633&width=1100',
    'https://kalles-5.myshopify.com/cdn/shop/files/ragbo4535819dd2_q5_2-0_09147857-3836-4dbf-a6b4-777b1738b821.jpg?v=1717404633&width=1100',
    'https://kalles-5.myshopify.com/cdn/shop/files/ragbo4535819dd2_q6_2-0_7fa82538-4fc6-415d-b489-5a1c1dcee344.jpg?v=1717404633&width=1100'
  ];

  const product = {
    id: 'short-sleeved-hoodie-static',
    ...
    <div className="w-full bg-white font-sans text-zinc-900">
      ...
      <div className="border-b border-zinc-100">
        <div className="max-w-[1400px] mx-auto px-6 py-4 flex items-center justify-between">
          <nav className="flex items-center gap-2 text-[14px] text-zinc-500">
            ...
            <span className="hover:text-zinc-900 cursor-pointer text-zinc-400">Fashion 2</span>
            <ChevronRight size={14} className="text-zinc-300" />
            <span className="hover:text-zinc-900 cursor-pointer">Short Sleeved Hoodie</span>
          </nav>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-10">
        <div className="flex flex-col lg:flex-row gap-12 items-start">
          ...
          {/* Left: Images (Main Sliding Carousel + Bottom Thumbnails) */}
          <div className="w-full lg:w-[45%]">
             {/* Main Image Slider Wrapper */}
             <div 
               className="w-full relative overflow-hidden bg-zinc-50 group"
             ...
             >
                ...
                >
                  {productImages.map((img, idx) => (
                    <div 
                      key={idx} 
                      className="min-w-full relative cursor-crosshair overflow-hidden animate-rotate-in-manual"
                ...
                <button 
                  onClick={prevImage}
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white text-zinc-900 flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100 transition-all hover:bg-cyan-400 hover:text-white z-10"
                >
                  <ChevronLeft size={24} />
                </button>
                <button 
                  onClick={nextImage}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white text-zinc-900 flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100 transition-all hover:bg-cyan-400 hover:text-white z-10"
                >
                  <ChevronRight size={24} />
                </button>

                {/* Dots indicator for mobile visibility */}
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 lg:hidden">
                  {productImages.map((_, idx) => (
                    <div 
                      key={idx}
                      className={`w-1.5 h-1.5 rounded-full transition-all ${activeIndex === idx ? 'bg-zinc-900 w-4' : 'bg-zinc-300'}`}
                    />
                  ))}
                </div>
             ...
             <div className="flex gap-4 mt-4 overflow-x-auto pb-2 no-scrollbar">
                {productImages.map((img, idx) => (
                  <div 
                    ...
                    className={`min-w-[100px] w-[100px] cursor-pointer border-2 transition-all animate-rotate-in-manual ${
                      activeIndex === idx ? 'border-zinc-900 shadow-md' : 'border-transparent hover:border-zinc-200'
                    }`}
                    style={{ animationDelay: `${idx * 0.15}s` }}
                  >
                    <img src={img} alt={`Thumb ${idx}`} className="w-full aspect-square object-cover" />
                  </div>
                ))}
             </div>
          </div>
          ...
          <div className="w-full lg:w-[55%] lg:sticky lg:top-10 relative">
            {/* Zoom Portal */}
            {zoomData.active && (
              <div 
            ...
            <div className="flex flex-col gap-6">
              <div>
                <h1 className="text-[24px] md:text-[32px] font-bold tracking-tight mb-2">Short Sleeved Hoodie</h1>
                <div className="flex items-center gap-2">
                  <span className="text-[20px] md:text-[24px] text-zinc-400 line-through font-medium">$45.00</span>
                  <span className="text-[20px] md:text-[24px] text-red-500 font-medium">$30.00</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="text-[13px] font-bold uppercase tracking-wider">
                  SIZE: <span className="font-normal text-zinc-400 ml-1">{selectedSize}</span>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                <button 
                  ...
                  className="flex-1 min-w-[200px] h-12 bg-[#67CFDE] hover:bg-[#55B6C4] text-white font-bold uppercase tracking-widest text-[12px] rounded-full transition-all flex items-center justify-center"
                >
                  ADD TO CART
                </button>
              ...
              <div className="space-y-1 text-[13px] pt-4 border-t border-zinc-50">
                 <div><span className="text-zinc-400">Availability:</span> <span className="text-zinc-600 italic">In stock</span></div>
                 <div><span className="text-zinc-400">SKU:</span></div>
                 <div><span className="text-zinc-400">Categories:</span> <span className="text-zinc-600">All, Best Seller, Bottoms, Denim, Dress, Fashion 2, Featured</span></div>
                 <div><span className="text-zinc-400">Tags:</span> <span className="text-zinc-600 italic">Price $7 - $50, Size L, Size M, Size S, Vendor Kalles, women</span></div>
              </div>
      ...
      <div className="w-full border-t border-zinc-100 py-16 mt-10">
        <div className="max-w-[1200px] mx-auto px-6">
          <div className="flex flex-wrap items-center justify-center gap-4 md:gap-12 mb-12">
            {['Description', 'Additional Information', 'Tab Custom HTML'].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`text-[14px] md:text-[16px] font-bold transition-all px-8 py-2.5 rounded-full border 
                  ${activeTab === tab 
                    ? 'border-zinc-900 text-zinc-900 bg-white shadow-md' 
            ...
            {activeTab === 'Additional Information' && (
              <div className="border border-zinc-100 overflow-hidden">
                <table className="w-full border-collapse">
                  <tbody>
                    <tr>
                      <th className="w-[150px] md:w-[200px] text-left py-4 px-6 font-bold text-zinc-900 border-r border-zinc-100 uppercase">Size</th>
                      <td className="py-4 px-6">S, M, L, XL</td>
                    </tr>
                  </tbody>
                </table>
              </div>
